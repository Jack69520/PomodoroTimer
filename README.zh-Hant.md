...
- **完全離線**：應用自身不連網，執行時不會向開發者或第三方伺服器上傳或共享使用者資料
- **本機儲存**：計時記錄、待辦、統計與帳戶資訊等主要業務資料保存在裝置本機 Room 資料庫；主題、鈴聲等裝置級偏好保存在 SharedPreferences
- **系統備份**：若使用者在裝置系統設定中開啟自動備份/雲備份，部分本機資料可能由作業系統備份至廠商或 Google 備份服務，詳見應用內隱私政策
