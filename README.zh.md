...
- **完全离线**：应用自身不联网，运行时不会向开发者或第三方服务器上传或共享用户数据
- **本地存储**：计时记录、待办、统计与账户信息等主要业务数据保存在设备本地 Room 数据库；主题、铃声等设备级偏好保存在 SharedPreferences
- **系统备份**：若用户在设备系统设置中开启自动备份/云备份，部分本地数据可能由操作系统备份至厂商或 Google 备份服务，详见应用内隐私政策
