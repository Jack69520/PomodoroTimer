...
- Landscape layout on the dedicated timer screen for a minimal, distraction-free UI
...
├── App.java / AppContainer.java / AppDatabase.java / MainActivity.java
├── data/           # Room entities, DAOs, repositories
├── domain/         # App-blocking policy
├── security/       # Password hashing
├── service/        # Timer foreground service, app blocking service
├── ui/             # Activities, fragments, ViewModels
│   ├── bootstrap/  # App startup bootstrap
...
- **Fully offline**: the app does not use the network at runtime and does not upload or share user data with the developer or third-party servers
- **Local storage**: sessions, todos, stats, and account data live in a local Room database; device-level preferences such as theme and ringtone are stored in SharedPreferences
- **System backup**: if you enable auto backup or cloud backup in device settings, some local data may be backed up by the OS to OEM or Google backup services—see the in-app Privacy Policy for details
