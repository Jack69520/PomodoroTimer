    ...
    public void getTodayStats(StatsCallback callback) {
        AppExecutors.getInstance().diskIo(() -> {
            try {
                ...
                int count = sessionDao.getCompletedCountInRangeForUser(userId, startOfDay, endOfDay);
                long duration = sessionDao.getTotalDurationInRangeForUser(userId, startOfDay, endOfDay);
