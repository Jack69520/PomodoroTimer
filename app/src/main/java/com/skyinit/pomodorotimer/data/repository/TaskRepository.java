...
public class TaskRepository {
    ...
    public void runOnDisk(Runnable runnable) {
        diskIo.execute(() -> {
            try {
                ...
                AppLog.e(TAG, "Task repository disk operation failed", throwable);
            }
        });
    }
    ...
    /**
     * 删除待办及其子任务。关联的 {@link com.skyinit.pomodorotimer.data.entity.PomodoroSession}
     * 记录<strong>不会</strong>被删除——计时统计以会话表为准，taskId 仅作历史引用。
     */
