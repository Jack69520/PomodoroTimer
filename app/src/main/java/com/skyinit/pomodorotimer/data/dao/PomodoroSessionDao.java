...
public interface PomodoroSessionDao {
    ...
    /**
     * 日历/列表展示用：排除同一轮专注中因暂停产生的未完成快照（存在同 startTime 的已完成记录时）。
     */
    @Query("SELECT * FROM pomodoro_sessions WHERE userId = :userId AND startTime >= :startTime AND startTime <= :endTime "
            + "AND NOT (completed = 0 AND EXISTS ("
            + "SELECT 1 FROM pomodoro_sessions AS twin WHERE twin.userId = pomodoro_sessions.userId "
            + "AND twin.startTime = pomodoro_sessions.startTime AND twin.completed = 1)) "
            + "ORDER BY startTime DESC")
    LiveData<List<PomodoroSession>> getDisplayableSessionsByDateRange(String userId, long startTime, long endTime);

    @Query("DELETE FROM pomodoro_sessions WHERE userId = :userId AND startTime = :startTime AND completed = 0")
    void deleteIncompleteSessionsByStartTime(String userId, long startTime);

    @Query("SELECT * FROM pomodoro_sessions WHERE userId = :userId AND completed = 1 "
            + "AND startTime >= :startTime AND startTime <= :endTime "
            + "AND (pauseCount > 0 OR (pauseReasons IS NOT NULL AND pauseReasons != '') "
            + "OR (pauseReason IS NOT NULL AND pauseReason != ''))")
    List<PomodoroSession> getCompletedSessionsWithPausesInRangeSync(String userId, long startTime, long endTime);
