 ...
 * 首次启动注册页 ViewModel。
 */
public class FirstRegisterViewModel extends AndroidViewModel {

    private final UserSessionRepository sessionRepository;
    private final MutableLiveData<Boolean> loading = new MutableLiveData<>(false);
    private final SingleLiveEvent<String> toastMessage = new SingleLiveEvent<>();
    private final SingleLiveEvent<String> registerSuccessUserId = new SingleLiveEvent<>();
    private final SingleLiveEvent<FormFieldError> fieldError = new SingleLiveEvent<>();

    public FirstRegisterViewModel(@NonNull Application application,
    ...
    public LiveData<String> getRegisterSuccessUserId() {
        return registerSuccessUserId;
    }
    ...
    public void register(String nickname, String password, String confirmPassword) {
        ...
        sessionRepository.register(trimmedNickname, trimmedPassword, trimmedConfirm, null, "",
                new AccountManager.RegisterCallback() {
                    @Override
                    public void onSuccess(User user) {
                        loading.setValue(false);
                        registerSuccessUserId.setValue(user.userId);
                    }
