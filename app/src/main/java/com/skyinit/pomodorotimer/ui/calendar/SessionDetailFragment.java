...
public class SessionDetailFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        MaterialButton viewBlockRecordsBtn = view.findViewById(R.id.view_block_records_btn);

        saveNotesBtn.setOnClickListener(v -> saveNotes());
        if (viewBlockRecordsBtn != null) {
            viewBlockRecordsBtn.setOnClickListener(v -> openBlockRecords());
        }
    ...
    private void bindSession(PomodoroSession session) {
        ...
        if (session.completed) {
            statusLabel = getString(R.string.session_detail_completed);
        } else if (SessionPauseUtils.isTimeoutFailure(
                session.pauseReason,
                session.pauseReasons,
                getString(R.string.timer_pause_reason_timeout))) {
            statusLabel = getString(R.string.session_detail_failed);
        } else {
            statusLabel = getString(R.string.session_detail_incomplete);
        }
        setDetailRow(R.id.row_status, R.string.session_detail_completion_status, statusLabel);
        ...
        setDetailRow(R.id.row_block_count, R.string.session_detail_block_count,
                String.valueOf(Math.max(0, session.blockEventCount)));
    ...
    private void openBlockRecords() {
        if (currentSession == null || getActivity() == null) {
            return;
        }
        Intent intent = new Intent(getActivity(), SessionBlockRecordsActivity.class);
        intent.putExtra(SessionBlockRecordsActivity.EXTRA_SESSION_START_TIME, currentSession.startTime);
        intent.putExtra(SessionBlockRecordsActivity.EXTRA_SESSION_END_TIME, currentSession.endTime);
